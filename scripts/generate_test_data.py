...
"""
生成测试数据脚本
使用方法: python manage.py shell < scripts/generate_test_data.py
或者: python manage.py shell
然后: exec(open('scripts/generate_test_data.py').read())
"""
...
def generate_products():
    ...
    for data in products_data:
        ...
        print(f"  创建产品: {product.item_name}")
...
def generate_orders(customers, products):
    ...
    for i in range(5):
        ...
        orders.append(order)
        print(f"  创建订单: {order_number} - 客户: {customer.name}")
        ...
        for product in selected_products:
            ...
            print(f"    添加订单项: {product.item_name} x {quantity} @ {unit_price}")
...
def generate_deliveries(orders):
    ...
    for i, order in enumerate(orders[:3]):
        ...
        print(f"  创建发货单: {delivery_number or '未填写'} - 订单: {order.order_number}")
        ...
        for order_item in order_items:
            ...
            print(f"    添加发货项: {order_item.product.item_name} x {delivered_quantity}")
...
def main():
    ...
    print(f"产品数量: {Products.objects.count()}")
    print(f"订单数量: {Order.objects.count()}")
    print(f"订单项数量: {OrderItem.objects.count()}")
    print(f"发货单数量: {Delivery.objects.count()}")
    print(f"发货项数量: {DeliveryItem.objects.count()}")
    print("=" * 50)
